...
- 集成相关
    - <a href="#小程序用户关闭麦克风再开启麦克风其他小程序用户无法收到麦克风开启事件">小程序用户关闭麦克风再开启麦克风，其他小程序用户无法收到麦克风开启事件</a>
...
::: note note
若客户端调用 [`Client.join`](https://doc.yunxin.163.com/docs/interface/NERTC_SDK/Latest/MiniApp/Client.html#join__anchor) 方法后未成功加入房间，服务端可能也会创建一个新的音视频房间。
:::
...
this.client.unpublish('audio').then(url => {
    ...
    //步骤一：将推流的 URL 设置为null，这样 live-pusher 组件就会停止渲染。

   //步骤二：我们只关闭了麦克风，而不关闭摄像头，所以需要立即将URL设置回去，以重新渲染 live-pusher 组件。同时，需要记得将 live-pusher 组件中的 muted 设置为 true，这样重新渲染的 live-pusher 组件就不会推送音频流。
...
云信音视频sdk在通话过程中会回调通话中的各种状态信息，包括本地音频流统计信息、本地视频流统计信息、通话中远端音频流的统计信息、通话中远端视频流的统计信息、当前通话统计等，各端的实现方式如下：
...
3. 接收端接受的是大流还是小流，是小流的话可以调用接口申请大流关闭小流。
