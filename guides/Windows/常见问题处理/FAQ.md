...
- 集成相关
    - <a href="#如何减小集成SDK后的App包体积？">如何减小集成 SDK 后的 App 包体积？</a>
...
::: note note
若客户端调用 [`joinChannel`](https://doc.yunxin.163.com/nertc/api-refer/windows/doxygen/Latest/zh/html/classnertc_1_1_i_rtc_engine.html#acc3f404cee7cd1b56b5019776dbb660a) 方法后未成功加入房间，服务端可能也会创建一个新的音视频房间。
:::
...
若您希望减小包体积，网易云信提供以下三种方式供您参考。

- 使用纯音频版本的 NERTC SDK。

    请前往[网易云信 SDK 下载中心](https://doc.yunxin.163.com/nertc/sdk-download)根据需要下载指定版本的纯音频包。
    
- 使用提供插件化动态库的 NERTC SDK（自 v4.6.20 起提供）。

    自 V4.6.20 起，NERTC SDK 支持以插件化方式集成美颜、虚拟背景等功能，提升 SDK 集成的灵活性与易操作性，您根据需要自行选择是否集成对应特性的动态库，以实现轻量接入裁剪包，具体集成方式请参考[集成 SDK](https://doc.yunxin.163.com/nertc/guide/jYwNjM5ODA?platform=windows)。

- 联系网易云信技术支持提供定制裁剪包。
    
    您可以根据实际场景中使用功能的差异，裁剪不需要的功能项。目前可裁剪的功能项包括：视频通话、耳返、AI 降噪、AI 超分、美声变声、H265 和 VP8 视频编解码技术、Protocol Buffers 协议、SCTP 协议等。具体请联系网易云信技术支持为您提供定制化服务。
...
<br> Windows 端的实现方式请参考如下代码：
  ```
  继承 `IRtcMediaStatsObserver` 类，实现如下回调：
...
2. 尝试通过 4G 连接或者其他 Wifi 信号排除网络问题。
