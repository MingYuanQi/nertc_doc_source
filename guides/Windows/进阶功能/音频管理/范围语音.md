...
- **TeamID**
    ...
    - 如果某成员使用 `team_id = 1` 进入语音房间，当他设置语音模式为 `NERtcRangeAudioModeTeam `，则只有 `team_id = 1` 的成员能听到他的声音；如果他设置的语音模式为 `NERtcRangeAudioModeDefault `，则除了  team_id = 1 的成员，一定范围内的玩家也能听到他的声音。
    ...
    <table>
        ...
        <tr>
            <td rowspan="2">team_id != 0，假设 team_id = 1 </td><td>NERtcRangeAudioModeTeam  </td><td>10米</td><td>声音只能和 team_id = 1 的成员互通</td>
        </tr>
        <tr>
            <td>NERtcRangeAudioModeDefault  </td><td>10米</td><td>声音能和 team_id = 1 的成员、以及语音模式设置为 NERtcRangeAudioModeDefault   的同房间10米范围内成员互通</td>
        </tr>
    </table>
...
在加入房间前，调用 [`setAudioRecvRange`](https://doc.yunxin.163.com/nertc/api-refer/windows/doxygen/Latest/zh/html/classnertc_1_1_i_rtc_engine_ex.html#aa2a512b95a1f7e63f7badbcee4bcf9c8) 接口设置语音范围。
...
### 步骤2 设置 TeamID
在加入房间前，调用 [`setRangeAudioTeamID`](https://doc.yunxin.163.com/nertc/api-refer/windows/doxygen/Latest/zh/html/classnertc_1_1_i_rtc_engine_ex.html#a59d6a3682f8caf0a269fbbadd98c490f) 接口设置队伍号。
...
在加入房间前，调用 [`setRangeAudioMode`](https://doc.yunxin.163.com/nertc/api-refer/windows/doxygen/Latest/zh/html/classnertc_1_1_i_rtc_engine_ex.html#a07611bb7ed19996d07325c7a7f25c87f) 接口设置语音模式。
...
在加入房间后，每帧调用 [`updateSelfPosition`](https://doc.yunxin.163.com/nertc/api-refer/windows/doxygen/Latest/zh/html/classnertc_1_1_i_rtc_engine_ex.html#a7d1e68f02ce3128fd85837bd431cce1f) 方法并设置 info 参数，以更新音源或听者的空间位置信息。
...
```
// step 0: 创建并初始化好引擎 rtc_engine_

// step 1: 入会前设置范围语音相关参数
rtc_engine_->setAudioRecvRange(audible_distance, conversational_distance, roll_off); // 主要设置 audible_distance, 其他参数设置默认值即可
rtc_engine_->setRangeAudioMode(mode);
rtc_engine_->setRangeAudioTeamID(team_id);

// step 2: 入会，进行能力协商
rtc_engine_->JoinChannel(token, channel_name, uid, channel_options);

// step 3: 会中本地用户的空间坐标
NERtcPositionInfo info;
// update speaker_position ...
rtc_engine_->updateSelfPosition(info);

// step 4: [可选] 会中动态更新范围语音参数
rtc_engine_->setAudioRecvRange(audible_distance2, conversational_distance, roll_off)
rtc_engine_->setRangeAudioMode(mode2);
rtc_engine_->setRangeAudioTeamID(team_id2);
...
| 文档 | 描述 | 
| ---- | -------------- | 
| [空间音效](https://doc.yunxin.163.com/nertc/guide/zA5MzYwNzE?platform=windows) | 如果需要更加沉浸式的体验，建议范围语音配合 3D 音效一起使用。| 
