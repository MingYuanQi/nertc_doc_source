...
2. 排查发送端的问题。
    1. 检查用户 B 是否已调用 [` publish `](https://doc.yunxin.163.com/nertc/api-refer/web/typedoc/Latest/zh/html/interfaces/client.client-1.html#publish)方法发布本地视频流，具体请参见[设置本地视图](https://doc.yunxin.163.com/docs/jcyOTA0ODM/TA1NTk0NzY?platformId=50082#4.设置本地视图)。
    ...
    3. 在服务监控平台查看视频上行帧率，如果上行帧率远低于 [`setVideoProfile`](https://doc.yunxin.163.com/nertc/api-refer/web/typedoc/Latest/zh/html/interfaces/stream.stream-1.html#setvideoprofile) 中设置的帧率，可能是相机有问题或手机性能有问题。


3. 排查接收端的问题。
    1. 检查用户 A 是否已调用 [`subscribe`](https://doc.yunxin.163.com/nertc/api-refer/web/typedoc/Latest/zh/html/interfaces/client.client-1.html#subscribe)订阅用户 B 的视频。
    2. 检查用户 A 是否已调用 [`setRemoteRenderMode`](https://doc.yunxin.163.com/nertc/api-refer/web/typedoc/Latest/zh/html/interfaces/stream.stream-1.html#setremoterendermode)设置用户 B 的画布。
...
1. 查看 [`setVideoProfile`](https://doc.yunxin.163.com/nertc/api-refer/web/typedoc/Latest/zh/html/interfaces/stream.stream-1.html#setvideoprofile) 中设置的分辨率是否和您的应用场景相匹配。
...
2. 在服务监控平台查看相关的通话质量数据，具体步骤请参见[监控通话质量](#监控通话质量)。
    1. 查看发送端的视频上行码率和接收端的下行码率，确认接收端接收的是大流还是小流。
        - 如果订阅的是小流，视频略微模糊是符合预期的，您可以调用[`setSubscribeConfig`](https://doc.yunxin.163.com/nertc/api-refer/web/typedoc/Latest/zh/html/interfaces/stream.stream-1.html#setsubscribeconfig)接口，将`highOrLow`参数的值设置为`NERTC.STREAM_TYPE.HIGH`，订阅大流。
        - 如果订阅的是大流，请继续执行下一步。
...
2. 在服务监控平台查看相关的通话质量数据，具体步骤请参见[监控通话质量](#监控通话质量)。
    ...
    1. 查看视频上行帧率，如果上行帧率太低，请查看 [`setVideoProfile`](https://doc.yunxin.163.com/nertc/api-refer/web/typedoc/Latest/zh/html/interfaces/stream.stream-1.html#setvideoprofile) 设置的帧率是否正常。
