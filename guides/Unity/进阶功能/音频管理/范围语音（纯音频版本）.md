...
- 您也可以在加入房间（[`JoinChannel`](https://doc.yunxin.163.com/nertc/api-refer/unity/doxygen/Latest/zh/audioOnly/html/classnertc_1_1_i_rtc_engine.html#ac2389c30485d409489442caf4268b246)）时，设置小队ID（teamID）、范围语音的模式（mode）和范围语音的接收范围（audibleDistance）。
:::
...
private void EnableAudioRange(){
    ...
    rtcEngine.SetRangeAudioTeamID(teamID);
    rtcEngine.SetRangeAudioMode(RtcRangeAudioMode.kNERtcRangeAudioModeTeam);//设置你的语音模式
    rtcEngine.SetAudioRecvRange(50, 1, RtcDistanceRolloffModel.kNERtcDistanceRolloffLinear);
    ...
    rtcEngine.SetAudioProfile(RtcAudioProfileType.kNERtcAudioProfileStandardExtend, RtcAudioScenarioType.kNERtcAudioScenarioChatroom);
#else
    rtcEngine.SetAudioProfile(RtcAudioProfileType.kNERtcAudioProfileStandardExtend, RtcAudioScenarioType.kNERtcAudioScenarioMusic);
#endif
}
...
| 文档 | 描述 | 
| ---- | -------------- | 
| [空间音效（纯音频版本）](/docs/jcyOTA0ODM/zY2MDU2MDI) | 如果需要更加沉浸式的体验，建议范围语音配合 3D 音效一起使用。| 
