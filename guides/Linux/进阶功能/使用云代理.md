...
- 公有云方式部署在云信公有云服务器上，可以降低用户 IT 岗位运维工作的维护成本，只需要配置云代理服务器白名单，不再需要根据云信服务器的增加而多次配置。
    ...
    ::: note note
    仅 V5.5.0 及之后版本的 NERTC SDK 支持此功能，请根据需要前往[网易云信 SDK 下载中心](https://doc.yunxin.163.com/nertc/sdk-download)获取最新版本的 SDK。
    :::
...
2. 调用 SDK API。
    ...
    1. 调用 [`setCloudProxy`](https://doc.yunxin.163.com/nertc/api-refer/linux/doxygen/Latest/zh/html/classnertc_1_1_i_rtc_engine_ex.html#ade9d3ad8490b2b17492b150773041948) 接口，开通云代理设置。

        成功连接云代理后，SDK 会触发 [`onConnectionStateChange`](https://doc.yunxin.163.com/nertc/api-refer/linux/doxygen/Latest/zh/html/classnertc_1_1_i_rtc_engine_event_handler.html#acc3468170eeb2e05915fc1df964823f5) 回调。
    ::: note note
    若您需要关闭云代理，请在加入房间前再次调用此接口，并将 `proxyType` 参数设置为 [`kNERtcTransportTypeNoneProxy(0)`](https://doc.yunxin.163.com/nertc/api-refer/linux/doxygen/Latest/zh/html/namespacenertc.html#af06cc939805f67adf02bd92b8fa47847a47feb7a1a7a904041ab9484bdb28d73d)，否则房间内的此项操作只在下次加入房间时生效。
    :::
    ...
    ```   
    // 第1步，开启云代理功能
    rtcEngine->setCloudProxy(kNERtcTransportTypeUDPProxy)

    // 第2步，加入房间
    rtcEngine->joinChannel(token , channelName, userID);

    //关闭云代理
    rtcEngine->setCloudProxy(kNERtcTransportTypeNoneProxy)
    ```
