...
## 配置步骤
1. 调用 [`enableVideoCorrection`](https://doc.yunxin.163.com/nertc/references/flutter/dartdoc/Latest/zh/nertc/NERtcEngine/enableVideoCorrection.html) 方法开启视频图像畸变矫正。调用此方法时，您设置 enable 参数为 true 开启视频矫正功能。
2. 调用 [`setVideoCorrectionConfig`](https://doc.yunxin.163.com/nertc/references/flutter/dartdoc/Latest/zh/nertc/NERtcEngine/setVideoCorrectionConfig.html) 方法设置视频矫正参数。相关参数的含义如下表所示。
| 参数 | 参数说明 |
|---|---|
...
|  | - enableMirror：是否镜像显示，在使用外部视频渲染时需要传入此参数。|

3. 调用 [enableVideoCorrection](https://doc.yunxin.163.com/nertc/references/flutter/dartdoc/Latest/zh/nertc/NERtcEngine/enableVideoCorrection.html) 方法关闭视频图像畸变矫正。调用此方法时，您需要设置 enable 参数为 false 关闭视频矫正功能。
 ...
 * 开启视频图像矫正功能
 */
int ret = await _engine.enableVideoCorrection(true);
/**
 * 关闭视频图像矫正功能
 */
int ret = await _engine.enableVideoCorrection(false);
...
int ret = await _engine.setVideoCorrectionConfig(config);

/**
 * 清空矫正参数
 */
int ret = await _engine.setVideoCorrectionConfig(null);
```
 
## API 参考
| **方法** | **功能描述**|
|:--|:--|
|[enableVideoCorrection](https://doc.yunxin.163.com/nertc/references/flutter/dartdoc/Latest/zh/nertc/NERtcEngine/enableVideoCorrection.html)|开启或关闭视频图像畸变矫正|
|[setVideoCorrectionConfig](https://doc.yunxin.163.com/nertc/references/flutter/dartdoc/Latest/zh/nertc/NERtcEngine/setVideoCorrectionConfig.html)|设置视频图像矫正参数|
