...
2. 排查发送端的问题。
    1. 检查用户 B 是否已调用 [`enableLocalVideo`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine-p.html#a2ea18a0b1c2fc5eb7f175dd24cdf0f2a)开启本地视频采集。

        - true： 表示开启本地视频采集。 
        - false： 表示关闭本地视频采集。
    2. 检查用户 B 的摄像头是否打开失败，如果日志中有 `no camera permission`报错，可以尝试更换或重启手机测试。
    3. 在服务监控平台查看视频上行帧率，如果上行帧率远低于 [`setLocalVideoConfig`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine-p.html#a040ee549ad4a2b1d74d275defda38140) 中设置的帧率，可能是相机有问题或手机性能有问题。


3. 排查接收端的问题。
    1. 检查用户 A 是否已调用 [`subscribeRemoteVideo:forUserID:streamType`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine_ex-p.html#a93b8bf74c51be46e6ed1f901a65cebfb)订阅用户 B 的视频。
    2. 检查用户 A 是否已调用 [`setupRemoteVideoCanvas`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine-p.html#ae56db50d6e25fac43db491fbfcf9a03a)设置用户 B 的画布。
...
1. 查看 [`setLocalVideoConfig`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine-p.html#a040ee549ad4a2b1d74d275defda38140) 中设置的分辨率是否和您的应用场景相匹配。
...
2. 在服务监控平台查看相关的通话质量数据，具体步骤请参见[监控通话质量](#监控通话质量)。
    1. 查看发送端的视频上行码率和接收端的下行码率，确认接收端接收的是大流还是小流。
        - 如果订阅的是小流，视频略微模糊是符合预期的，您可以调用[`subscribeRemoteVideo`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine_ex-p.html#a93b8bf74c51be46e6ed1f901a65cebfb)接口，将[`streamType`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/_n_e_rtc_engine_enum_8h.html#a9fa1ba5b58874cf4948cc33d18bff7a7)参数的值设置为`kNERtcRemoteVideoStreamTypeHigh `，订阅大流。
        - 如果订阅的是大流，请继续执行下一步。
...
2. 在服务监控平台查看相关的通话质量数据，具体步骤请参见[监控通话质量](#监控通话质量)。
    ...
    1. 查看视频上行帧率，如果上行帧率太低，请查看 [`setLocalVideoConfig`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine-p.html#a040ee549ad4a2b1d74d275defda38140) 设置的帧率是否正常。
