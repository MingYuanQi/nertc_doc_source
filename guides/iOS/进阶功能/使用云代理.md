...
2. 调用 SDK API。
    ...
    1. 调用 [`setCloudProxy`](/docs/interface/NERTC_SDK/V4.4.301/iOS/html/protocol_i_n_e_rtc_engine_ex-p.html#a935fede5cf099d6cdefb4990182def07) 接口，开通云代理设置。

        成功连接云代理后，SDK 会触发 [`onNERtcEngineConnectionStateChangeWithState(kNERtcConnectionStateConnecting, kNERtcReasonConnectionChangedSettingProxyServer)`](/docs/interface/NERTC_SDK/V4.4.301/iOS/html/protocol_n_e_rtc_engine_delegate-p.html#afec9bad19bbc5109c7cf1f161d246907) 回调。
    ::: note note
    若您需要关闭云代理，请在加入房间前再次调用此接口，并将 `proxyType` 参数设置为 [`NERtcTransportTypeNoneProxy(0)`](/docs/interface/NERTC_SDK/V4.4.301/iOS/html/_n_e_rtc_engine_enum_8h.html#ad975c8874d1cf7e370e25bd27e89f168)，否则房间内的此项操作只在下次加入房间时生效。
    :::
    2. 调用 [`joinChannelWithToken`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine-p.html#adf60d9392e4b50ff73872138965f022f) 接口加入房间，此时 SDK 会使用云代理地址进行加房间操作。
    ::: note notice
    ...
    //关闭云代理
    [[NERtcEngine sharedEngine] setCloudProxy:NERtcTransportTypeNoneProxy];
    ```
