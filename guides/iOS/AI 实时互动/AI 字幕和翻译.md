...
2. **集成 NERTC SDK**：项目已集成最新版本的 NERTC SDK，推荐 5.6.50 及以上版本。详情请参考 [集成 SDK（音视频）](https://doc.yunxin.163.com/nertc/guide/TM5NzI5MjI?platform=iOS)。
3. **初始化 NERTC SDK**：在使用实时字幕和字幕翻译功能前，初始化 NERTC SDK 并将用户加入音视频房间。详情请参考 [实现音视频通话](https://doc.yunxin.163.com/nertc/guide/DA0MzExMTY?platform=iOS#%E7%AC%AC%E4%B8%89%E6%AD%A5%E5%88%9D%E5%A7%8B%E5%8C%96)。
    ...
    //加入 RTC 房间，加入成功后，会收到 onJoinChannel 回调
...
您可以在完成准备工作后，调用 [`startASRCaption`](https://doc.yunxin.163.com/nertc/references/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine_ex-p.html#a7706561b76cfc84ca6da9b83fb3b05f2) 接口开启实时字幕功能。功能调用流程如下图所示，涉及的接口请参考下文 [相关接口](#apiList) 章节：
...
sequenceDiagram
    ...
    SDK->>Server: 请求开启字幕服务
    Server-->>SDK: 返回字幕服务状态
    SDK-->>App: 4. 监听回调，返回字幕开启结果<br> (onNERtcEngineAsrCaptionStateChanged)
    Server->>SDK: 发送字幕内容
    SDK-->>App: 监听回调，返回字幕状态和内容<br> (onNERtcEngineAsrCaptionResult)
    end
```
...
| 参数 | 说明 |
| --- | --- |
| [`NERtcASRCaptionConfig`](https://doc.yunxin.163.com/nertc/references/iOS/doxygen/Latest/zh/html/interface_n_e_rtc_a_s_r_caption_config.html) | 字幕语言配置对象。 |
...
| 接口/事件 | 说明 |
| --- | --- |
| [`joinChannel`](https://doc.yunxin.163.com/nertc/api-refer/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine-p.html#adf60d9392e4b50ff73872138965f022f) | 加入音视频房间。 |
| [`startASRCaption`](https://doc.yunxin.163.com/nertc/references/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine_ex-p.html#a7706561b76cfc84ca6da9b83fb3b05f2) | 开启实时字幕功能。 |
| [`stopASRCaption`](https://doc.yunxin.163.com/nertc/references/iOS/doxygen/Latest/zh/html/protocol_i_n_e_rtc_engine_ex-p.html#a64d0c0b703406998f70378ae3b511864) | 关闭实时字幕功能。 |
| [`onNERtcEngineUserDidJoinWithUserID`](https://doc.yunxin.163.com/nertc/references/iOS/doxygen/Latest/zh/html/protocol_n_e_rtc_engine_delegate-p.html#a7e47ea04b33f9dc7746ede6a0d53a6eb) | 远端用户加入房间的回调。 |
| [`onNERtcEngineAsrCaptionStateChanged`](https://doc.yunxin.163.com/nertc/references/iOS/doxygen/Latest/zh/html/protocol_n_e_rtc_engine_delegate_ex-p.html#a7f49d9bd0d278aab3591ebd4f77b10e6) | 字幕状态变化的回调。 |
| [`onNERtcEngineAsrCaptionResult`](https://doc.yunxin.163.com/nertc/references/iOS/doxygen/Latest/zh/html/protocol_n_e_rtc_engine_delegate_ex-p.html#ae3f51a1bfe620acfe8aca45df9874c57) | 收到字幕内容的回调。 |
...
- 在 `onNERtcEngineAsrCaptionResult` 回调中获取字幕内容后，使用自定义 UI 组件展示字幕。